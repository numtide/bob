...
impl Backend for RustBackend {
    ...
    fn resolve_attr(&self, target: &str, repo_root: &Path) -> Option<String> {
        // `<crate>`           → `rust.workspaceMembers.<crate>.build`
        // `<profile>.<crate>` → `rust.<profile>.workspaceMembers.<crate>.build`
        //
        // Every backend owns one top-level bob.nix attr named after `id()`;
        // rust profiles live *under* `rust` so a profile name can't collide
        // with another backend's attr (`cc`, `go`, …). The profile prefix
        // lets a repo's bob.nix expose several cargoNix instances (e.g.
        // prod-tuned vs dev-loop flags) without bob needing to know what
        // "profile" means — it's just an attr-path segment. The crate part
        // is still gated on Cargo.toml membership so unknown names fall
        ...
        members.contains_key(crate_name).then(|| match prefix {
            Some(p) => format!("rust.{p}.workspaceMembers.{crate_name}.build"),
            None => format!("rust.workspaceMembers.{crate_name}.build"),
        })
    }
